...
def CSV_logger(data, csv_file_path):
...
for size in range(50, 80, 5):
    ...
    BASE_NAME = f"dummy_{NUM_ORBS}x{NUM_SATS_PER_ORB}"
    NICE_NAME = f"Dummy-{NUM_ORBS}x{NUM_SATS_PER_ORB}"
    ...
    # chosen_pairs = [
    #     (f"{BASE_NAME}_isls", 903, 933, "TcpNewReno", full_satellite_network_isls),
    ...
    for gid in range(50):
        chosen_pairs.append(
            (f"{BASE_NAME}_isls", (NUM_ORBS*NUM_SATS_PER_ORB)+gid, (NUM_ORBS *
             NUM_SATS_PER_ORB)+gid+50, "TcpNewReno", full_satellite_network_isls),
        )
